...
DuiMarkupNode::DuiMarkupNode() : m_pOwner(NULL)
{
}

DuiMarkupNode::DuiMarkupNode(DuiMarkup* pOwner, int iPos) : m_pOwner(pOwner), m_iPos(iPos), m_nAttributes(0)
{
}

DuiMarkupNode DuiMarkupNode::GetSibling()
{
    if( m_pOwner == NULL ) return DuiMarkupNode();
    ULONG iPos = m_pOwner->m_pElements[m_iPos].iNext;
    if( iPos == 0 ) return DuiMarkupNode();
    return DuiMarkupNode(m_pOwner, iPos);
}

bool DuiMarkupNode::HasSiblings() const
{
...
DuiMarkupNode DuiMarkupNode::GetChild()
{
    if( m_pOwner == NULL ) return DuiMarkupNode();
...
DuiMarkupNode DuiMarkupNode::GetChild(LPCTSTR pstrName)
{
    if( m_pOwner == NULL ) return DuiMarkupNode();
    ULONG iPos = m_pOwner->m_pElements[m_iPos].iChild;
    while( iPos != 0 ) {
    ...
    return DuiMarkupNode();
}

bool DuiMarkupNode::HasChildren() const
{
    if( m_pOwner == NULL ) return false;
    return m_pOwner->m_pElements[m_iPos].iChild != 0;
}

DuiMarkupNode DuiMarkupNode::GetParent()
{
    if( m_pOwner == NULL ) return DuiMarkupNode();
...
LPCTSTR DuiMarkupNode::GetAttributeValue(int iIndex)
{
    if( m_pOwner == NULL ) return NULL;
    if( m_nAttributes == 0 ) _MapAttributes();
    if( iIndex < 0 || iIndex >= m_nAttributes ) return _T("");
...
LPCTSTR DuiMarkupNode::GetAttributeValue(LPCTSTR pstrName)
{
    if( m_pOwner == NULL ) return NULL;
    if( m_nAttributes == 0 ) _MapAttributes();
...
bool DuiMarkupNode::GetAttributeValue(int iIndex, LPTSTR pstrValue, SIZE_T cchMax)
{
    if( m_pOwner == NULL ) return false;
    if( m_nAttributes == 0 ) _MapAttributes();
...
bool DuiMarkupNode::GetAttributeValue(LPCTSTR pstrName, LPTSTR pstrValue, SIZE_T cchMax)
{
    if( m_pOwner == NULL ) return false;
    if( m_nAttributes == 0 ) _MapAttributes();
    for( int i = 0; i < m_nAttributes; i++ ) {
...
int DuiMarkupNode::GetAttributeCount()
{
...
bool DuiMarkupNode::HasAttributes()
{
    if( m_pOwner == NULL ) return false;
    if( m_nAttributes == 0 ) _MapAttributes();
...
bool DuiMarkupNode::HasAttribute(LPCTSTR pstrName)
{
    if( m_pOwner == NULL ) return false;
    if( m_nAttributes == 0 ) _MapAttributes();
    for( int i = 0; i < m_nAttributes; i++ ) {
...
void DuiMarkupNode::_MapAttributes()
{
    m_nAttributes = 0;
...
DuiMarkup::DuiMarkup(LPCTSTR pstrXML)
{
    m_pstrXML = NULL;
    m_pElements = NULL;
    m_nElements = 0;
...
bool DuiMarkup::IsValid() const
{
    return m_pElements != NULL;
}

void DuiMarkup::SetPreserveWhitespace(bool bPreserve)
{
    m_bPreserveWhitespace = bPreserve;
}

bool DuiMarkup::Load(LPCTSTR pstrXML)
{
    Release();
...
bool DuiMarkup::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding)
{
#ifdef _UNICODE
...
bool DuiMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    Release();
...
void DuiMarkup::GetLastErrorMessage(LPTSTR pstrMessage, SIZE_T cchMax) const
{
    _tcsncpy(pstrMessage, m_szErrorMsg, cchMax);
}

void DuiMarkup::GetLastErrorLocation(LPTSTR pstrSource, SIZE_T cchMax) const
{
    _tcsncpy(pstrSource, m_szErrorXML, cchMax);
}

DuiMarkupNode DuiMarkup::GetRoot()
{
    if( m_nElements == 0 ) return DuiMarkupNode();
    return DuiMarkupNode(this, 1);
}

bool DuiMarkup::_Parse()
{
...
bool DuiMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
...
DuiMarkup::XMLELEMENT* DuiMarkup::_ReserveElement()
{
...
void DuiMarkup::_SkipWhitespace(LPCTSTR& pstr) const
{
    while( *pstr > _T('\0') && *pstr <= _T(' ') ) pstr = ::CharNext(pstr);
}

void DuiMarkup::_SkipWhitespace(LPTSTR& pstr) const
{
    while( *pstr > _T('\0') && *pstr <= _T(' ') ) pstr = ::CharNext(pstr);
}

void DuiMarkup::_SkipIdentifier(LPCTSTR& pstr) const
{
    // 属性只能用英文，所以这样处理没有问题
    while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || _istalnum(*pstr)) ) pstr = ::CharNext(pstr);
}

void DuiMarkup::_SkipIdentifier(LPTSTR& pstr) const
{
    // 属性只能用英文，所以这样处理没有问题
    while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || _istalnum(*pstr)) ) pstr = ::CharNext(pstr);
}

bool DuiMarkup::_ParseAttributes(LPTSTR& pstrText)
{   
...
bool DuiMarkup::_ParseData(LPTSTR& pstrText, LPTSTR& pstrDest, char cEnd)
{
...
void DuiMarkup::_ParseMetaChar(LPTSTR& pstrText, LPTSTR& pstrDest)
{
...
bool DuiMarkup::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation)
{
    // Register last error
