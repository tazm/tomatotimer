...
{
    m_dwNormalColor = RGB(0xbb, 0xbb, 0xbb);
    m_dwHoveredColor = RGB(0xff, 0x30, 0x00);
    m_dwPressedColor = RGB(0xff, 0x30, 0x00);
    m_bHovered = false;
    m_bPressed = false;
}
...
{
    if( _tcscmp(pstrName, _T("Hyperlink")) == 0 ) return static_cast<CHyperlinkUI*>(this);
    return CLabelUI::GetInterface(pstrName);
}

UINT CHyperlinkUI::GetControlFlags() const
{
    return ( IsEnabled() && !m_sUrl.IsEmpty() ) ? UIFLAG_SETCURSOR : 0;
}

void CHyperlinkUI::SetUrl(LPCTSTR pstrUrl)
{
    if( 0 != StrCmpNI(pstrUrl, _T("http"), 4 ))
    {
        m_sUrl = _T("http://");
    }
    else
...
void CHyperlinkUI::SetColor(DWORD dwNormalColor, DWORD dwHoveredColor, DWORD dwPressedColor)
{
    if( dwNormalColor ) m_dwNormalColor = dwNormalColor;
    if( dwHoveredColor ) m_dwHoveredColor = dwHoveredColor;
    if( dwPressedColor ) m_dwPressedColor = dwPressedColor;
}
...
void CHyperlinkUI::PaintText(HDC hDC)
{
    DWORD dwColor = m_bPressed ? m_dwPressedColor : m_bHovered ? m_dwHoveredColor : m_dwNormalColor;
    RECT rc = m_rcItem;
    rc.left += m_rcTextPadding.left;
    rc.right -= m_rcTextPadding.right;
    rc.top += m_rcTextPadding.top;
...
void CHyperlinkUI::BrowseUrl()
{
    ...
    ShellExecute(NULL, L"open", m_sUrl.GetData(), NULL, NULL, SW_MAXIMIZE);
}
...
{
    if( _tcscmp(pstrName, _T("Picture")) == 0 ) return static_cast<CPictureUI*>(this);
    return CLabelUI::GetInterface(pstrName);
}

void CPictureUI::SetPicture(LPCTSTR pstrUrl)
{
    if( !PathFileExists(pstrUrl) )
    {
        return;
    }

    m_sPicture = pstrUrl;
    TCHAR tszBkimg[1024] = {0};
    wnsprintf(tszBkimg, RTL_NUMBER_OF(tszBkimg), _T("file='%s' restype='external'"), pstrUrl);
    SetBkImage(tszBkimg);
}
