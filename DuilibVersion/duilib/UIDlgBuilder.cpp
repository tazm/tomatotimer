...
{
    m_pCallback = pCallback;
    DuiMarkupNode root = m_xml.GetRoot();
    ...
    if( pManager ) {
        ...
        LPTSTR pstr = NULL;
        for( DuiMarkupNode node = root.GetChild() ; node.IsValid(); node = node.GetSibling() ) {
...
DuiMarkup* CDialogBuilder::GetMarkup()
{
    return &m_xml;
}
...
CControlUI* CDialogBuilder::_Parse(DuiMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    CDialogLayoutUI* pDialogLayout = NULL;
    IContainerUI* pContainer = NULL;
    CControlUI* pReturn = NULL;
    for( DuiMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
        ...
        if( _tcscmp(pstrClass, _T("Include")) == 0 ) {
            ...
            case 7:
                ...
                else if( _tcscmp(pstrClass, _T("Picture")) == 0 )           pControl = new CPictureUI;
                break;
            case 8:
            ...
            case 9:
                ...
                else if( _tcscmp(pstrClass, _T("Hyperlink")) == 0 )         pControl = new CHyperlinkUI;
                break;
            case 10:
