					...
					DWORD dwAlignment/* = eMenuAlignment_Left | eMenuAlignment_Top*/)
{
	...
	Create((m_pOwner == NULL) ? pMainPaintManager->GetPaintWindow() : m_pOwner->GetManager()->GetPaintWindow(), NULL, WS_POPUP , WS_EX_TOOLWINDOW | WS_EX_TOPMOST, CRect());
	...
	if( event.Type == UIEVENT_BUTTONUP )
	{
		if( IsEnabled() ){
			...
			else
			{
				...
				if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
				{
					CStdString* strPost = new CStdString(GetName().GetData());
...
void CMenuElementUI::SetChecked(bool bCheck/* = true*/)
{
	if (!m_bCheckItem || CMenuWnd::GetGlobalContextMenuObserver().GetMenuCheckInfo() == NULL )
		return;
	std::map<CStdString,bool>::iterator it = CMenuWnd::GetGlobalContextMenuObserver().GetMenuCheckInfo()->find(GetName());
	if (it == CMenuWnd::GetGlobalContextMenuObserver().GetMenuCheckInfo()->end())
		CMenuWnd::GetGlobalContextMenuObserver().GetMenuCheckInfo()->insert(std::map<CStdString,bool>::value_type(GetName(),bCheck));
	else
		it->second = bCheck;
