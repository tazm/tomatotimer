...
class UILIB_API DuiMarkup
{
    friend DuiMarkupNode;
public:
    DuiMarkup(LPCTSTR pstrXML = NULL);
    ~DuiMarkup();
...
class UILIB_API DuiMarkupNode
{
    friend DuiMarkup;
private:
    DuiMarkupNode();
    DuiMarkupNode(DuiMarkup* pOwner, int iPos);

public:
    bool IsValid() const;

    DuiMarkupNode GetParent();
    DuiMarkupNode GetSibling();
    DuiMarkupNode GetChild();
    DuiMarkupNode GetChild(LPCTSTR pstrName);
