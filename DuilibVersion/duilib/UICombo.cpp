...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        OutputDebugStringA("CComboWnd : HandleMessage");
        POINT pt = { 0 };
        ::GetCursorPos(&pt);
        ::ScreenToClient(m_pm.GetPaintWindow(), &pt);
        CControlUI* pControl = m_pm.FindControl(pt);
        if( pControl && _tcscmp(pControl->GetClass(), _T("ListBtnElementUI")) == 0)
        {
            if (m_pOwner->m_pManager != NULL)
            {
                m_pOwner->m_pManager->SendNotify(m_pOwner, _T("userselect"), NULL, NULL);
            }
        }
...
bool CComboUI::DeleteBtnClick(int iIndex)
{
    if (m_pManager != NULL)
    {
        m_pManager->SendNotify(this, TEXT("itemdelete"), iIndex, NULL);
        Invalidate();
        return true;
    }
...
CStdString CComboUI::GetCurItemPswdText() const
{
    if (m_iCurSel < 0) return _T("");
    CListBtnElementUI* pElement = static_cast<CListBtnElementUI*>(m_items[m_iCurSel]);
    return pElement->GetPswdString();
}

bool CComboUI::GetCurItemState() const
{
    if (m_iCurSel < 0) return false;
    CListBtnElementUI* pElement = static_cast<CListBtnElementUI*>(m_items[m_iCurSel]);
    return pElement->GetItemState();
}

bool CComboUI::IsCurItemDeleted() const
{
    if (m_iCurSel < 0) return true;
    CListBtnElementUI* pElement = static_cast<CListBtnElementUI*>(m_items[m_iCurSel]);
    return pElement->IsItemDeleted();
}
