...
bool CListUI::DeleteBtnClick(int iIndex)
{
    return true;
}
...
bool CListElementUI::RemoveItem()
{
    if (!IsEnabled()) return false;
    if (m_pOwner != NULL)
    {
        m_pOwner->DeleteBtnClick(m_iIndex);
        SetVisible(false);
        return true;
    }
...
//
//
CListBtnElementUI::CListBtnElementUI()
{
    m_bDelete = false;
}
...
LPCTSTR CListBtnElementUI::GetClass() const
{
    return _T("ListBtnElementUI");
}

LPVOID CListBtnElementUI::GetInterface(LPCTSTR pstrName)
{
    if (_tcscmp(pstrName, _T("ListBtnElement")) == 0)
        return static_cast<CListBtnElementUI*>(this);
    return CListElementUI::GetInterface(pstrName);
}

void CListBtnElementUI::SetPswdString(LPCTSTR strPswd)
{
    m_pswdStr = strPswd;
}

void CListBtnElementUI::SetItemState(bool bRemembered)
{
    m_bRememberPswd = bRemembered;
}

CStdString CListBtnElementUI::GetPswdString() const
{
    return m_pswdStr;
}

bool CListBtnElementUI::GetItemState() const
{
    return m_bRememberPswd;
}

bool CListBtnElementUI::IsItemDeleted() const
{
    return m_bDelete;
}

void CListBtnElementUI::InitializeDeletedState()
{
    m_bDelete = false;
}

void CListBtnElementUI::DoEvent(TEventUI& event)
{
    if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND)
    {
        if (m_pOwner != NULL)
    ...
    if (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN)
    {
        if (IsEnabled())
        {
            ...
            if (p.x > m_rcCross.left - 3 && p.x < m_rcCross.right + 3 && p.y < m_rcCross.bottom + 3 && p.y >  m_rcCross.top - 3)
            {
                m_bDelete = true;
                RemoveItem();
            }
...
SIZE CListBtnElementUI::EstimateSize(SIZE szAvailable)
{
    if( m_pOwner == NULL ) return CSize(0, 0);
...
void CListBtnElementUI::DrawDeleteCross(HDC hDC, const RECT& rcCross)
{
    ASSERT(m_pOwner);
    if (m_pOwner == NULL)
        return;
    TListInfoUI* pInfo = m_pOwner->GetListInfo();
    if (pInfo->dwTextColor != 0)
    {
        RECT rcLine1 = {rcCross.left, rcCross.top, rcCross.right + 1, rcCross.bottom + 1};
        CRenderEngine::DrawLine(hDC, rcLine1, 1, GetAdjustColor(pInfo->dwTextColor));
        RECT rcLine2 = {rcCross.left, rcCross.bottom, rcCross.right + 1, rcCross.top - 1};
        CRenderEngine::DrawLine(hDC, rcLine2, 1, GetAdjustColor(pInfo->dwTextColor));
    }
}

void CListBtnElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    if( m_sText.IsEmpty() ) return;
...
void CListBtnElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
    ...
    int nRight = (rcItem.GetWidth() > rcItem.GetHeight() ? rcItem.right - rcItem.GetHeight() : rcItem.left);
    rcItem.right = nRight;
    RECT rcText(m_rcItem);
    rcText.right = nRight;
    DrawItemText(hDC, rcText);

    m_rcCross.left = m_rcItem.right - 22;
    m_rcCross.top = (m_rcItem.top + m_rcItem.bottom) / 2 - 3;
    m_rcCross.right = m_rcCross.left + 6;
    m_rcCross.bottom = m_rcCross.top + 6;
    m_rcCross.Normalize();
    DrawDeleteCross(hDC, m_rcCross);
}
/////////////////////////////////////////////////////////////////////////////////////
//
//
